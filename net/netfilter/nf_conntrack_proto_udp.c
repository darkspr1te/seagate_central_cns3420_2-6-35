...
#undef CNS3XXX_PPE_UDP_DEBUG
#define SPPE_START_UDP_FLOW_BY_PACKET_COUNT
#ifdef SPPE_START_UDP_FLOW_BY_PACKET_COUNT
#include <net/netfilter/nf_conntrack_acct.h>
#define SPPE_START_UDP_FLOW_THRESHOLD 10
#endif
#endif
...
#if defined (CONFIG_CNS3XXX_SPPE)
static int sppe_udp_flow_add_ipv4_fl(struct nf_conn *ct)
{
	...
	return sppe_func_hook(&param);
}

static int sppe_udp_flow_add_ipv4_fw(struct nf_conn *ct)
{
	SPPE_PARAM param;
	struct nf_conntrack_tuple *orig, *reply;
...
{
	if (sppe_udp_flow_add_ipv4_fl(ct)) {
#ifdef CNS3XXX_PPE_UDP_DEBUG
		printk("fail to add IPv4 UDP from-WAN flow!!\n");
#endif
	}

	if (sppe_udp_flow_add_ipv4_fw(ct)) {
#ifdef CNS3XXX_PPE_UDP_DEBUG
		printk("fail to add IPv4 UDP from-WAN flow!!\n");
#endif
	}
...
static int sppe_udp_flow_add_ipv6(struct nf_conn *ct)
{
	...
	if (sppe_func_hook(&param)) {
#ifdef CNS3XXX_PPE_UDP_DEBUG
		printk("<%s> fail to add IPv6 UDP from-WAN flow!!\n", __FUNCTION__);
#endif
	}
		      ...
		      unsigned int hooknum)
{
	...
	if (test_bit(IPS_SEEN_REPLY_BIT, &ct->status)) {
		...
		if (sppe_hook_mode) {
			...
			if (acct) {
				if (acct[CTINFO2DIR(ctinfo)].packets > SPPE_START_UDP_FLOW_THRESHOLD) {
					sppe_udp_flow_add_ipv4_fl(ct);
				}
			}
		}
#endif
