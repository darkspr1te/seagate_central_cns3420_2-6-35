...
#undef CNS3XXX_PPE_PPTP_GRE_DEBUG
#endif
...
static int sppe_gre_flow_add(struct nf_conn *ct)
{
...
#ifdef CNS3XXX_PPE_PPTP_GRE_DEBUG
        printk("<%s> fail to add IPv4 PPTP-GRE from-LAN flow!!\n", __FUNCTION__);
#endif
    }
    ...
    if (sppe_func_hook(&param)) {
#ifdef CNS3XXX_PPE_PPTP_GRE_DEBUG
        printk("<%s> fail to add IPv4 PPTP-GRE from-WAN flow!!\n", __FUNCTION__);
#endif
    }
