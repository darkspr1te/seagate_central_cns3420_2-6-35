 ...
 */
int button_ioctl(struct inode *inode, struct file *file,
		 unsigned int cmd, unsigned long arg)
{
	...
	case BIOC_GET_EVENT:
		interruptible_sleep_on(&b_wait);
		mutex_lock(&b_mutex);
		if (button_event) {
			copy_to_user(argp, &button_event, sizeof(button_event));
			button_event = 0;
		}
...
void button_timer(unsigned long _data)
{
	/* schedule work */
...
static int button_gpio_init(void)
{
	...
	MISC_GPIOB_PIN_ENABLE_REG &= ~(0x1 << 2);
	...
	setup_timer(&bdata->timer, button_timer, 0);
	INIT_WORK(&bdata->work, gpio_button_work_func);

	request_irq(gpio_to_irq(FACTORY_DEFAULT), button_gpio_intr_handler,
