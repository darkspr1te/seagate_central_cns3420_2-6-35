...
#include "../cns3xxx_pse_init/cns3xxx_symbol.h"
#include "../cns3xxx_pse_init/cns3xxx.h"
#include "cns3xxx_tool.h"

#ifdef CONFIG_CNS3XXX_PSE_SW_PHY_AUTO_POLLING
extern u8 cns3xxx_sw_auto_polling_reg(u8 mac_port);
extern int cns3xxx_std_phy_set_force(u16 phy_addr, u16 speed, u8 duplex);
#endif
extern int cns3xxx_std_phy_AN_enable(u16 phy_addr, int en);
...
#ifdef CONFIG_CNS3XXX_PSE_SW_PHY_AUTO_POLLING
	sw_poll_reg = cns3xxx_sw_auto_polling_reg(priv->net_device_priv->which_port);
	// sw auto polling enable for the port
	if ((sw_poll_reg >> 5) & 0x1)
	{
		pause->autoneg = (((sw_poll_reg >> 6) & 0x1) ? AUTONEG_ENABLE : AUTONEG_DISABLE);
		pause->tx_pause = (mac_port_config >> 12) & 1;
		pause->rx_pause = (mac_port_config >> 11) & 1;
	}
#endif
}
...
static int cns3xxx_set_pauseparam(struct net_device *netdev, struct ethtool_pauseparam *pause)
{
	...
	if ((sw_poll_reg >> 5) & 0x1)
	{
		phy_addr = (sw_poll_reg & 0x1f);
		cns3xxx_std_phy_AN_enable(phy_addr, pause->autoneg);
	}
#else
	GET_PORT_PHY_ADDR(priv->net_device_priv->which_port, phy_addr)
	cns3xxx_std_phy_AN_enable(phy_addr, pause->autoneg);
#endif
	return 0;
}

extern  u32 bcm53115M_get_link_status(int);
...
u32 cns3xxx_get_link(struct net_device *netdev)
{
	...
	switch (priv->net_device_priv->which_port)
	{
		case MAC_PORT0:
		{
                        ...
                        mac_port_config = bcm53115M_get_link_status(MAC_PORT0);
#else
                        mac_port_config = MAC0_CFG_REG;
#endif
			break;
		...
		{
#if (defined(CONFIG_VB) || defined (CONFIG_VB_2))
                        mac_port_config = bcm53115M_get_link_status(MAC_PORT1);
#else
                        mac_port_config = MAC1_CFG_REG;
#endif
			break;
...
#ifdef CONFIG_CNS3XXX_PSE_SW_PHY_AUTO_POLLING
	mac_port_config = cns3xxx_sw_auto_polling_reg(priv->net_device_priv->which_port);
	// sw auto polling enable for the port
	if ((mac_port_config >> 5) & 0x1)
		return ((mac_port_config >> 7) & 0x1);
#endif
...
#ifdef CONFIG_CNS3XXX_PSE_SW_PHY_AUTO_POLLING
	value = cns3xxx_sw_auto_polling_reg(priv->net_device_priv->which_port);
	// sw auto polling enable for the port
	if ((value >> 5) & 0x1)
	{
		ecmd->autoneg = ((value >> 6) & 0x1) ? AUTONEG_ENABLE : AUTONEG_DISABLE;
		ecmd->duplex = ((mac_port_config >> 10) & 0x1) ? DUPLEX_FULL : DUPLEX_HALF;
		switch ((mac_port_config >> 8) & 0x3)
		{
			case 0:
				ecmd->speed = SPEED_10;
				break;
...
static int cns3xxx_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	if ((value >> 5) & 0x1)
	{
		phy_addr = (value & 0x1f);
		if (ecmd->autoneg == AUTONEG_ENABLE)
			cns3xxx_std_phy_AN_enable(phy_addr, 1);
		else
			cns3xxx_std_phy_set_force(phy_addr, ecmd->speed, ecmd->duplex);
	}
#else
	GET_PORT_PHY_ADDR(priv->net_device_priv->which_port, phy_addr)
	cns3xxx_std_phy_AN_enable(phy_addr, (ecmd->autoneg == AUTONEG_ENABLE) ? 1 : 0);
#endif
	// down then up
	return 0;
}
