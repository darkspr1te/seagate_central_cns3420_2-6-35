...
#define MISC_GPIOA_PIN_EN		MISC_MEM_MAP_VALUE(0x014)

#define PM_MEM_MAP_VALUE(reg_offset)	(*((uint32_t volatile *)(CNS3XXX_PM_BASE_VIRT + reg_offset)))
#define PM_PLL_LCD_CTRL			PM_MEM_MAP_VALUE(0x018)

#define LCDC_MEM_MAP_VALUE(reg_offset)	(*((uint32_t volatile *)(CNS3XXX_CLCD_BASE_VIRT + reg_offset)))
#define LCDC_CLK_LCD_CTRL		LCDC_MEM_MAP_VALUE(0x008)

#define LCDC_CLK_BYBASS_ON()		(LCDC_CLK_LCD_CTRL |= 0x04000000)
#define LCDC_CLK_BYBASS_OFF()		(LCDC_CLK_LCD_CTRL &= ~0x04000000)
...
//		fb->fb.var.xres_virtual != cached_fb_var.yres_virtual ||	
...
//	if(clcdfb_is_fb_changed(fb)) {
		...
		/* FPGA has only a fix clock */
		//clk_set_rate(fb->clk, (1000000000 / regs.pixclock) * 1000);

		PM_PLL_LCD_CTRL &= 0xFFFF0000;
		switch(regs.pixclock) {
		case 39721: //vga...
			PM_PLL_LCD_CTRL |= 0x2A84; //25MHz...
			LCDC_CLK_BYBASS_ON();
			break;
		case 25000: //svga...
			PM_PLL_LCD_CTRL |= 0x2C04; //40MHz...
			LCDC_CLK_BYBASS_ON();
			break;
		case 15384: //xga...
			PM_PLL_LCD_CTRL |= 0x2B43;	//65MHz...
			LCDC_CLK_BYBASS_ON();
			break;
		case 30030: //7 inch touch panel
			PM_PLL_LCD_CTRL |= 0x2B5C;	//33.3MHz...
			LCDC_CLK_BYBASS_ON();
			break;
		default: // 4 inch lcd panel
			PM_PLL_LCD_CTRL |= 0xCC83;	//18MHz...
			LCDC_CLK_BYBASS_OFF();
			break;
		}
...
static void cns3xxx_lcdc_init_pwr_up(void)
{
	MISC_IO_CTRL |= 0x2;
	MISC_GPIOA_PIN_EN |= 0x200;
...
static int clcdfb_probe(struct amba_device *dev, struct amba_id *id)
{
	...
	if (ret == 0) {
		...
		/* Need to set mode to turn on LCD panel*/
