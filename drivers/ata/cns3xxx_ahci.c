...
#include <mach/pm.h>
#include <mach/cns3xxx.h> // plany
#include <mach/misc.h>
#include <linux/proc_fs.h> 

#define DRV_NAME	"cns3xxx-ahci"
...
#define SPINUP_DELAY_ENABLE

#if defined(SPINUP_DELAY_ENABLE)

#ifdef CONFIG_CIRRUS_DUAL_MTD_ENV

#include "../rtc/rtc-nvedit.h"

int fmg_get(const char *name, int *ret_len, char **uboot_env) {
    char *nbuf;
    int ret;

    /* sanity */

    if (NULL == name || NULL == ret_len || NULL == uboot_env) {
        printk(KERN_ERR\
    ...
    *uboot_env = NULL;
    ...
    ret = rtcnvet_get_env(nbuf, uboot_env);
 
    kfree(nbuf);
 
    if (0 == ret) {
        if (NULL != *uboot_env) {
           *ret_len = strlen(*uboot_env);
            return 0;
        }
    }
    printk(KERN_ERR\
...
#define PARTITION_NAME "SPI-UBootEnv"	/* refer to cns3xxx_spi_flash_partitions */

#define MTD_READ_LEN 1024
#define MTD_STR_LEN 18

static char mtd_str[MTD_READ_LEN], mtd_string[MTD_STR_LEN+1]={0};
...
int fmg_get(const char *name, int *ret_len, char *uboot_env)
{
	...
	memset(mtd_string,0x0,MTD_STR_LEN+1);
	...
	for(i=0;i<MTD_READ_LEN-nlen;i++) {
		...
		//printk("z = %d , nlen = %d\n",z,nlen);
		if(z==nlen) {
			memcpy(tmp_str,mtd_str+i+nlen,MTD_STR_LEN);
			tmp_str[MTD_STR_LEN] = '\0';
			//printk("tmp_str = [%s]\n",tmp_str);
			strncpy(mtd_string, tmp_str, MTD_STR_LEN);
			*ret_len = strlen(mtd_string);
			if (uboot_env != NULL)
				strncpy(uboot_env, mtd_string, strlen(mtd_string));
			return 0;
		}
	}
	return -1;
...
#endif //SPINUP_DELAY_ENABLE

/* I2C Define */
#define I2C_MEM_MAP_ADDR(x)                   (CNS3XXX_SSP_BASE_VIRT + x)
#define I2C_MEM_MAP_VALUE(x)		\
				(*((unsigned int volatile*)I2C_MEM_MAP_ADDR(x)))
#define I2C_CONTROLLER_REG                    I2C_MEM_MAP_VALUE(0x20)
#define I2C_TIMEOUT                           I2C_MEM_MAP_VALUE(0x24)
#define I2C_SLAVE_ADDRESS_REG                 I2C_MEM_MAP_VALUE(0x28)
#define I2C_WRITE_DATA_REG                    I2C_MEM_MAP_VALUE(0x2C)
#define CNS3xxx_I2C_START_TRANSFER()		\
				((I2C_CONTROLLER_REG) |= (0x1 << 6))

/* Use I2C to configure SATA phy TX AMP. */
static void cns3xxx_sata_phy_amp_config(void);
...
static ssize_t ahci_transmit_led_message(struct ata_port *ap, u32 state,
					ssize_t size);
#if defined(CONFIG_AMP_PURENAS) || defined(CONFIG_AMP_ALL)
void sata_offload_proc_init(void);
#endif
...
#ifdef CONFIG_PM
static int cns_ahci_suspend(struct platform_device *dev, pm_message_t state);
static int cns_ahci_resume(struct platform_device *dev);
#endif
...
#ifdef CONFIG_PM
	.suspend		= cns_ahci_suspend,
	.resume			= cns_ahci_resume,
#endif
};
#endif
...
/* KH:  20110628   move define to libata for module
 *      20110831   add sd_enable
 *
 **/
struct proc_dir_entry *sata_proc_entry;
extern long unsigned int cnsx_last_jiffs;
extern unsigned int  cnsx_sata_stagger_enable;
extern unsigned int  cnsx_sata_disk_standby[CNSX_STAGGER_SATA_PORT][CNSX_STAGGER_SATA_PORT_DISKS];
extern unsigned int  cnsx_sd_enable[CNSX_STAGGER_SATA_PORT];
extern unsigned char cnsx_sd_name[CNSX_STAGGER_SATA_PORT][4];
/*  ------------------------------------  */
static int sata_read_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
   {
      int i =0 ,j = 0;
      int num = 0;
      for(i=0;i<CNSX_STAGGER_SATA_PORT;i++)
         {
            if(cnsx_sd_enable[i]==1)
               {
                  num += sprintf(page + num ,"\nPORT [%02d]:\n------------------------------\n",i);
                  for(j=0;j<CNSX_STAGGER_SATA_PORT_DISKS;j++)
                     {
                        if(cnsx_sata_disk_standby[i][j]!=0)
                           {
                              if(cnsx_sata_disk_standby[i][j]==1)
                                 {
                                    num += sprintf(page + num , "  channel[%d] = /dev/%s - online\n",j,cnsx_sd_name[i]);
                                 }
                              else if(cnsx_sata_disk_standby[i][j]==2)
                                 {
                                    num += sprintf(page + num , "  channel[%d] = /dev/%s - standby/spindown\n",j,cnsx_sd_name[i]);
                                 }
                           }
                     }
                  num += sprintf(page + num ,"\n");
               ...
               {
                  num += sprintf(page + num ,"PORT [%02d]: -------[no sd disk]\n",i);
               }
            num += sprintf(page + num ,"\n");
#endif
         }
      num += sprintf(page + num , "\n\ncnsx_last_jiffs          = %lx\n",cnsx_last_jiffs);
      num += sprintf(page + num , "jiffies                  = %lx\n",jiffies);
      num += sprintf(page + num , "\n\ncnsx_sata_stagger_enable = %d\n\n",cnsx_sata_stagger_enable);

      return num;
   }
static int sata_write_proc(struct file *file, const char *buffer,
		unsigned long count, void *data)
{
	if (count) {
		/*
		unsigned int i = 0;
		unsigned int spindown = 0;
		*/
		printk("sata_write_proc\n");
		if (buffer[0] == 's' && buffer[1] == 't' && buffer[2] == 'a' &&
				buffer[3] == 'g' && buffer[4] == 'g' &&
				buffer[5] == 'e' && buffer[6] == 'r') {
			cnsx_sata_stagger_enable = simple_strtol(buffer + 8, 0, 0);

			if (cnsx_sata_stagger_enable >= 1)
				cnsx_sata_stagger_enable = 1;
			else
				cnsx_sata_stagger_enable = 0;


			/*
			for (i = 0; i < 5; i++) {
				if(sata_disk_standby[0][i] == 2)
					spindown++;
				if(sata_disk_standby[1][i] == 2)
					spindown++;
			}

			if (spindown != 0) {
				printk("can't set sata stagger %d, becasue not all device is spin-up\n",sata_stagger_enable);
				printk("all sata devices need spin-up , and now %d device spin-down\n",spindown);
				printk("\n\n");
				sata_stagger_enable = 0;
			}
			*/
			printk("\nset cnsx_sata_stagger_enable = %d\n\n", cnsx_sata_stagger_enable);
		}
	}
	return count;
}
			  ...
			  unsigned long deadline)
{
	...
	if ( pmp && ret ) ret = ahci_do_softreset(link, class, 0, deadline, ahci_check_ready);
	return ret;
}
...
#ifdef CONFIG_CIRRUS_DUAL_MTD_ENV
    char *mtd_string = NULL;

    if (0 == counter) {
        if (0 == fmg_get("model_name=", &val_len, &mtd_string)) {
            if (strstr(mtd_string, "whitney_4bay")) {
                if (ap->port_no) {
                    msleep(30*1000);
                    counter = 1;
                }   
...
#else /*CONFIG_CIRRUS_DUAL_MTD_ENV*/
	//printk("%s,%d: %d\n", __FUNCTION__, __LINE__, ap->port_no);
    if ((counter == 0) && (0 == init_mtd_env())) {
		if (0 == fmg_get("model_name=", &val_len, NULL)) {
			//printk("%s,%d: [%s]\n", __FUNCTION__, __LINE__, mtd_string);
			if(strstr(mtd_string, "whitney_4bay")) {
				if (ap->port_no) {
					msleep(30*1000);
					counter = 1;
...
static int cns_ahci_suspend(struct platform_device *dev, pm_message_t state)
{
	u32 reg;
	/* disable SATA dlock */
	cns3xxx_pwr_clk_disable(0x1 << PM_CLK_GATE_REG_OFFSET_SATA);

	/* power down SATA PHY */
	cns3xxx_pwr_power_down(0x1 << PM_PLL_HM_PD_CTRL_REG_OFFSET_SATA_PHY0);
	cns3xxx_pwr_power_down(0x1 << PM_PLL_HM_PD_CTRL_REG_OFFSET_SATA_PHY1);

	reg = MISC_SATA_POWER_MODE;
	reg &= ~(0x1<< 16);  // SLUMBER Mode
	reg &= ~(0x1<< 17);  // SLUMBER Mode 
	MISC_SATA_POWER_MODE = reg;
...
static int cns_ahci_resume(struct platform_device *dev)
{
	...
	reg = MISC_SATA_POWER_MODE;
	reg |= 0x1<< 16;  // Disable SATA PHY 0 from SLUMBER Mode
	reg |= 0x1<< 17;  // Disable SATA PHY 1 from SLUMBER Mode 
	MISC_SATA_POWER_MODE = reg;
...
#endif
        remove_proc_entry("sata",cns3xxx_proc_dir);/* KH */
        return 0;
}
...
static void cns3xxx_sata_phy_amp_config(void)
{
	/* Keep SATA PHY always at de-assert reset, MISC 0x60C and 0x610. */
	MISC_REG_VALUE(0x60c) |= (0x1 << 29);
	mdelay(5);
	MISC_REG_VALUE(0x610) |= (0x1 << 29);
	mdelay(5);

	/* Turn-on I2C interface of SATA PHY0. */
	MISC_REG_VALUE(0x310) |= (0x1 << 1);

	/* I2C */
	I2C_SLAVE_ADDRESS_REG = 0x70;
	I2C_WRITE_DATA_REG = 0x904;
	I2C_CONTROLLER_REG = 0x80000014;
	CNS3xxx_I2C_START_TRANSFER();
	while (1) {
		if ((I2C_CONTROLLER_REG & (0x1 << 6)) == 0)
			break;
	}
	mdelay(5);
	MISC_REG_VALUE(0x310) &= ~(0x1 << 1);
...
static int ahci_probe(struct platform_device *pdev) // plany
#endif
{
...
EXPORT_SYMBOL(sata_offload_proc_init);
#endif
...
static int __init ahci_init(void)
{
	...
	ahci_phy_init();
        /* KH */
        sata_proc_entry = create_proc_entry("sata",S_IFREG | S_IRUGO,cns3xxx_proc_dir);
	if (sata_proc_entry)
	   {
              sata_proc_entry->read_proc  = sata_read_proc;
              sata_proc_entry->write_proc = sata_write_proc;
           }
