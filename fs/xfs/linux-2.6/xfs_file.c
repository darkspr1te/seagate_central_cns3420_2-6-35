...
#ifdef CONFIG_AMP
extern ssize_t cvm_generic_file_splice_write(struct pipe_inode_info *pipe,
	...
	unsigned int		flags)
{
	...
	/*
 	 * If its AMP, call our function that does memcpy offload, otherwise
 	 * go with the default kernel version.
 	 */
#ifdef CONFIG_AMP
	ret = cvm_generic_file_splice_write(pipe, outfilp, ppos, count, flags);
#else
