...
#ifdef CONFIG_SPECULATIVE_PREFETCH_FIX
...
#ifdef CONFIG_AMP
void ___dma_page_cpu_to_dev_usb(struct page *page, unsigned long off,
	size_t size, enum dma_data_direction dir)
{
	unsigned long paddr;
	...
	if (!hard_smp_processor_id()) {
		/*
		 * 1) Invalidate the read buffer before submitting it
		 *    to USB driver.
		 *
		 * 2) In case of a write, skip the flush of data coming
		 *    from splice write. This is because we have
		 *    memcpy offload happening in DP core and relevant
		 *    data will be flushed from DP. CP core can skip
		 *    this flush so that we get good performance.
		 */

		if (dir == DMA_FROM_DEVICE)
			dma_cache_maint_page_local(page, off, size, dir);
		else {
			if (page->flags & (1 << PG_fs_usb_data))
				page->flags &= (~(1 << PG_fs_usb_data));
			else
				dma_cache_maint_page_local(page, off, size,
						dir);
		}
	}
	paddr = page_to_phys(page) + off;
...
EXPORT_SYMBOL(___dma_page_cpu_to_dev_usb);
#endif
...
static void dma_cache_maint_contiguous_local(struct page *page,
...
#ifdef CONFIG_AMP
int dma_map_sg_usb(struct device *dev, struct scatterlist *sg, int nents,
		enum dma_data_direction dir)
{
	struct scatterlist *s;
	int i, j;
	...
	for_each_sg(sg, s, nents, i) {
		s->dma_address = dma_map_page_usb(dev, sg_page(s), s->offset,
						s->length, dir);
		if (dma_mapping_error(dev, s->dma_address))
			goto bad_mapping;
	}
