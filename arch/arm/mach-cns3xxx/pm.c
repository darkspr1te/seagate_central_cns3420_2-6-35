...
static void cns3xxx_pwr_soft_rst_force(unsigned int block)
{
	 ...
	 */
	if (block & 0x30000001) {
		/* If bootloader is booting from SPI, SPI interface must be
		 * enabled */
...
#if defined(CONFIG_CNS3XXX_RESET_GPIO_BLOCK)
#else
	if (block & (0x1 << PM_SOFT_RST_REG_OFFST_GPIO))
		return;
#endif
...
/*
 * cns_pwr_pre_change()/cns_pwr_post_change()
 *
 */
#ifdef CONFIG_PCI
#define CNS_PWR_CLK_MASK_PCI	((0x1 << 17) | (0x1 << 18))
#endif

#define CNS_PWR_PLL_MASK	0
u32 clk_save;
u32 pll_save;

static void cns_pwr_pre_change(void)
{
	clk_save = readl(CNS3XXX_PM_BASE_VIRT + 0x00);
	pll_save = readl(CNS3XXX_PM_BASE_VIRT + 0x1C);

#ifdef CONFIG_PCI
	writel(clk_save & ~(CNS_PWR_CLK_MASK_PCI), CNS3XXX_PM_BASE_VIRT + 0x00);
#if 0
	printk(KERN_INFO "%s: clk_save 0x%.8x pll_save_0x%.8x\n",
			__func__, clk_save, pll_save);
	printk(KERN_INFO "%s: clk_save & ~(CNS_PWR_CLK_MASK) 0x%.8x pll_save | "
			"(CNS_PWR_PLL_MASK) 0x%.8x\n", __func__,
			clk_save & ~(CNS_PWR_CLK_MASK),
			pll_save | (CNS_PWR_PLL_MASK));
#endif
#endif
	writel(pll_save | (CNS_PWR_PLL_MASK), CNS3XXX_PM_BASE_VIRT + 0x1C);

}

static void cns_pwr_post_change(void)
{
	writel(pll_save, CNS3XXX_PM_BASE_VIRT + 0x1C);
	udelay(300); /* Wait for 300us for the PLL output clock locked. */
	writel(clk_save, CNS3XXX_PM_BASE_VIRT + 0x00);
}
...
void cns3xxx_pwr_change_cpu_clock(unsigned int cpu_sel, unsigned int div_sel)
{
	...
	if ((CNS3XXX_PWR_PLL_CPU_700MHZ < cpu_sel) ||
		...
		printk(KERN_INFO "%s: incorrect parameter, cpu_sel:%d, div_sel:"
				"%d\n", __func__, cpu_sel, div_sel);
		return;
	}
...
#define DATA_PLANE_CORE 1

int cnsx_dp_power_event = CNSX_DP_RESUME;
EXPORT_SYMBOL(cnsx_dp_power_event);
int cnsx_dp_power_event_ack = CNSX_DP_PWR_EVENT_ACK;
EXPORT_SYMBOL(cnsx_dp_power_event_ack);

void cns3xxx_secondary_suspend(void)
{
	cnsx_dp_power_event_ack = CNSX_DP_PWR_EVENT_PENDING;
	cnsx_dp_power_event = CNSX_DP_SUSPEND;
	wmb();
read_again:
	rmb();
	if (cnsx_dp_power_event_ack == CNSX_DP_PWR_EVENT_PENDING)
		goto read_again;
}
EXPORT_SYMBOL(cns3xxx_secondary_suspend);
...
void cns3xxx_secondary_resume(void)
{
	amp_call_function_single(DATA_PLANE_CORE, (void *)dummyDP, NULL, 1);
}
EXPORT_SYMBOL(cns3xxx_secondary_resume);
...
#ifdef CONFIG_AMP
	/* for AMP, RAID does'nt use the irq, so do not include the same for
	 * wakeup */
	writel(0xFFFFFFFB, CNS3XXX_PM_BASE_VIRT + 0x02C); /* PM_WU_CTRL1_REG */
#else
...
void (*disable_rtl_phy_action)(void) = NULL;
void (*enable_rtl_phy_action)(void) = NULL;
EXPORT_SYMBOL_GPL(disable_rtl_phy_action);
EXPORT_SYMBOL_GPL(enable_rtl_phy_action);
...
{
	if(disable_rtl_phy_action)
		disable_rtl_phy_action();
	...
	wakeup_clean();
	wakeup_set(IRQ_CNS3XXX_PMU);
	wakeup_set(IRQ_CNS3XXX_SW_STATUS);
	wakeup_set(IRQ_CNS3XXX_RTC);

	/* enable clock scaling interrupt */
	writel(0x00000001, CNS3XXX_TC11MP_GIC_DIST_BASE_VIRT + 0x104);

#ifdef CONFIG_AMP
	cns3xxx_secondary_suspend();
	printk(KERN_INFO "<0> CP-core moving to WFI\n");
#endif /* CONFIG_AMP */
...
#ifdef CONFIG_AMP
	printk(KERN_INFO "<0> CP-core out from WFI\n");
	cns3xxx_secondary_resume();
#endif /* CONFIG_AMP */

	cns3xxx_pwr_mode(CNS3XXX_PWR_CPU_MODE_DFS);

	if(enable_rtl_phy_action)
		enable_rtl_phy_action();
}
...
static int cns3xxx_read_proc(char *page, char **start, off_t off, int count,
		int *eof, void *data)
{
	int num = 0;
	...
	case 0:
		num += sprintf(page + num, "300MHz\n");
		break;
	case 1:
		num += sprintf(page + num, "333MHz\n");
		break;
	case 2:
		num += sprintf(page + num, "366MHz\n");
		break;
	case 3:
		num += sprintf(page + num, "400MHz\n");
		break;
	case 4:
		num += sprintf(page + num, "433MHz\n");
		break;
	case 5:
		num += sprintf(page + num, "466MHz\n");
		break;
	case 6:
		num += sprintf(page + num, "500MHz\n");
		break;
	case 7:
		num += sprintf(page + num, "533MHz\n");
		break;
	case 8:
		num += sprintf(page + num, "566MHz\n");
		break;
	case 9:
		num += sprintf(page + num, "600MHz\n");
		break;
	default:
...
static int pm_clk_write_proc(struct file *file, const char *buffer,
...
static int pm_clk_read_proc(char *page, char **start, off_t off, int count,
		int *eof, void *data)
{
...
static int pm_pll_pd_write_proc(struct file *file, const char *buffer,
		unsigned long count, void *data)
{
	if (count) {
...
static int pm_pll_pd_read_proc(char *page, char **start, off_t off, int count,
		int *eof, void *data)
{
...
{
	pmu_proc_entry = create_proc_entry("pmu", S_IFREG | S_IRUGO,
			cns3xxx_proc_dir);

	if (pmu_proc_entry)
		pmu_proc_entry->read_proc = cns3xxx_read_proc;

	pm_clk_proc_entry = create_proc_entry("pm_clk", S_IFREG | S_IRUGO,
			cns3xxx_proc_dir);
	...
	pm_pll_pd_proc_entry = create_proc_entry("pm_pll_pd", S_IFREG | S_IRUGO,
			cns3xxx_proc_dir);
