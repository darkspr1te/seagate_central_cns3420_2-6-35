...
void gic_unmask_irq(unsigned int irq)
{
	...
	spin_lock(&irq_controller_lock);
	writel(mask, gic_dist_base(irq) + GIC_DIST_ENABLE_SET +
			(gic_irq(irq) / 32) * 4);
	spin_unlock(&irq_controller_lock);
}
...
{
	void __iomem *reg = gic_dist_base(irq) + GIC_DIST_TARGET +
		(gic_irq(irq) & ~3);
 ...
 */
int set_interrupt_type_by_base(void __iomem *base, int id, u32 type)
{
 ...
 * 0 - level high active, 1 - rising edge sensitive
 */
int set_interrupt_type(int id, u32 type)
{
	set_interrupt_type_by_base(
			(void __iomem *) CNS3XXX_TC11MP_GIC_DIST_BASE_VIRT,
			id, type);
	return 0;
}
...
int set_interrupt_pri(u32 id, u32 pri)
{
	set_interrupt_pri_by_base(
			(void __iomem *) CNS3XXX_TC11MP_GIC_DIST_BASE_VIRT,
			id, pri);
	return 0;
}
...
int get_interrupt_pri(int id, u32 *pri)
{
	get_interrupt_pri_by_base(
			(void __iomem *) CNS3XXX_TC11MP_GIC_DIST_BASE_VIRT,
			id, pri);
	return 0;
}
...
int cns3xxx_write_pri_mask(u8 pri_mask)
{
	writel(pri_mask, (void __iomem *) CNS3XXX_TC11MP_GIC_CPU_BASE_VIRT +
			GIC_CPU_PRIMASK);
	return 0;
}
