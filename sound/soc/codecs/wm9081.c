...
static struct wm9081_priv *gpWm9081 = NULL;

void Wm9081DumpRegister(void)
{
	int i;
	
  printk("\n>>>>>>>>>> Dump Reg Cache<<<<<<<<<<<<\n");
	for(i=0; i<WM9081_MAX_REGISTER; i++) {
 		if((i%16 == 0) && i)
 			printk("\n");
		printk("%04x ", gpWm9081->reg_cache[i]);
	}
	printk("\n");
}
...
//		printk("%s=>%d: 8bit\n", __FUNCTION__, __LINE__);
		//Without Companding...
		aif1 |= WM9081_DAC_COMPMODE;
		aif1 &= ~WM9081_DAC_COMP;
		//A-law...
//		aif1 |= WM9081_DAC_COMPMODE;
//		aif1 |= WM9081_DAC_COMP;
		//u-law...
...
static int wm9081_set_tdm_slot(struct snd_soc_dai *dai, unsigned int tx_mask,
...
#define WM9081_FORMATS \
	(SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | \
				      ...
				      const struct i2c_device_id *id)
{
	...
	printk("%s=>%d\n", __FUNCTION__, __LINE__);
#endif
	gpWm9081 = kzalloc(sizeof(struct wm9081_priv), GFP_KERNEL);
	if (gpWm9081 == NULL)
		return -ENOMEM;

	codec = &gpWm9081->codec;
	codec->hw_write = (hw_write_t)i2c_master_send;
	gpWm9081->retune = i2c->dev.platform_data;

	i2c_set_clientdata(i2c, gpWm9081);
	codec->control_data = i2c;

	codec->dev = &i2c->dev;

	return wm9081_register(gpWm9081);
}
